...
setup(
    ...
    packages = ['pyNN','pyNN.nest', 'pyNN.pcsim', 'pyNN.neuron', 'pyNN.brian', 'pyNN.recording', 'pyNN.hardware', 'pyNN.hardware.spikey'],
    package_data = {'pyNN': ['hoc/*.hoc', 'hoc/*.mod'], 'pyNN.hardware.spikey': ['pyhal/*.py', 'config/*.py', 'config/*.xml', 'config/calibration/*', 'tools/*.py']},
...
PyNN is a work in progress, but is already being used for several large-scale simulation projects.""",
    ...
    keywords = "computational neuroscience simulation neuron nest pcsim brian neuroml spikey",
